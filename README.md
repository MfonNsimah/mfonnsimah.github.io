# mfonnsimah.github.io

# 👋 Hi, I’m Mfon Nsimah

### Data Analytics Educator | Curriculum Developer | Community Builder
**Helping beginners move from confusion to clarity through data storytelling.**

---

## 🔍 About Me
I specialize in turning raw data into actionable insights and teaching others the "why" before the "how". My work bridges the gap between technical complexity and practical application, ensuring that digital skills stick.

- 🎓 **Lead Instructor** at TechDucate Academy.
- 🌍 **Community Ambassador** at Data Science Nigeria (DSN).
- [span_0](start_span)[span_1](start_span)[span_2](start_span)🛠️ **Technical Supporter** for 3MTT Nigeria (Cross River State), supporting 5,000+ fellows[span_0](end_span)[span_1](end_span)[span_2](end_span).
- [span_3](start_span)[span_4](start_span)📊 **Curriculum Developer** focused on high-impact, learner-centered models[span_3](end_span)[span_4](end_span).

---

## 🧠 Tools & Tech Stack
| Category | Tools/Skills |
| :--- | :--- |
| **Languages** | SQL, Python (Pandas, NumPy) |
| **Data Viz** | Power BI (DAX, Modeling), Excel Dashboards |
| **AI Tools** | [span_5](start_span)[span_6](start_span)Microsoft Copilot (Data Analysis & Presentations)[span_5](end_span)[span_6](end_span) |
| **Education** | [span_7](start_span)[span_8](start_span)Instructional Design, Technical Writing, Curriculum Development[span_7](end_span)[span_8](end_span) |

---

## 📊 Featured Projects
> I build hands-on projects designed to teach core analytics concepts using realistic datasets.

### 1. [Beginner Data Analytics Series](https://github.com/yourusername/beginner-data-analytics)
A collection of end-to-end projects covering Excel fundamentals, SQL querying, and Power BI reporting.

### 2. [SQL Fundamentals for Beginners](https://github.com/yourusername/sql-fundamentals)
Structured, practice-focused notes and exercises designed for absolute beginners.

### 3. [Power BI DAX Guide](https://github.com/yourusername/power-bi-dax)
A beginner-friendly elementary tutorial for mastering DAX and data modeling.

---

[Power BI](https://app.powerbi.com/Redirect?action=openreport&context=Annotate&ctid=55b122fe-b3e0-4411-b1b6-1cbb3bd5a01d&pbi_source=mobile_android&groupObjectId=625c99c5-0955-4ec9-b13d-e9f87cf054b4&reportObjectId=52a94d70-8912-40f6-828f-fc5b87fa3c88&reportPage=03c1621616329de1303b&bookmarkGuid=b18a972d-a6ab-46b5-8570-a583f975720d&fullScreen=0)

---

## 📚 Publications & Resources
- **eBook:** Data Analysis for Beginners – A full tutorial-style guide.
- **Course:** Excel for Absolute Beginners – Mini-course + practical exercises.

---

## 🤝 Let's Connect
- [span_9](start_span)[span_10](start_span)💼 [LinkedIn](www.linkedin.com/in/mfon-nsimah-11846bb8)[span_9](end_span)[span_10](end_span)
- [span_11](start_span)[span_12](start_span)📧 [Email Me](mailto:mfonnsimah@gmail.com)[span_11](end_span)[span_12](end_span)
- 🌐 [TechDucate Academy]

---

> "Learning data shouldn’t feel intimidating. It should feel empowering."
